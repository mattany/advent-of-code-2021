...
def solution_1(arr):
    ...
    for d in digit_sums:
        if d > len(arr) / 2:
            ...
            res.append(False)
    return int("0b" + "".join(["1" if i else "0" for i in res]), 2) * int("0b" + "".join(["0" if i else "1" for i in
                                                                                          res]), 2)
...
def solution_2(arr):
    ...
    while len(oxygen_arr) > 1 or len(c02scrubber_arr) > 1:
        oxygen_zero_arr, oxygen_one_arr = [_ for _ in oxygen_arr if _[pos] == "0"], [_ for _ in oxygen_arr if
                                                                                     _[pos] == "1"]
        oxygen_arr = oxygen_one_arr if len(oxygen_one_arr) >= len(oxygen_zero_arr) else oxygen_zero_arr
        c02_zero_arr, c02_one_arr = [_ for _ in c02scrubber_arr if _[pos] == "0"], [_ for _ in c02scrubber_arr if
                                                                                    _[pos] == "1"]
